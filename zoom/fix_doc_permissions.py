"""
既存の文字起こしGoogleドキュメントの閲覧権限を一括変更するスクリプト
...
DOC_URL_COLUMN = "G"  # 文字起こしDocのURL列
...
def get_all_doc_urls_from_sheet(spreadsheet_id: str, sheet_name: str) -> list[str]:
    """シートからすべての文字起こしDocのURLを取得"""
    ...
    try:
        ...
        # G列のすべての値を取得（ヘッダー除く）
        col_values = worksheet.col_values(7)  # G列 = 7

        # ヘッダーをスキップしてURLのみ取得
        urls = [url for url in col_values[1:] if url and 'docs.google.com' in url]
        return urls
...
def set_public_permission(drive_service, doc_id: str) -> bool:
...
def main(dry_run: bool = False):
    ...
    for sheet_name in SHEET_NAMES:
        ...
        urls = get_all_doc_urls_from_sheet(SPREADSHEET_ID, sheet_name)
        print(f"  → {len(urls)}件のURLを取得")
        all_urls.extend(urls)

    # 重複を除去
    unique_urls = list(set(all_urls))
    print(f"\n合計: {len(unique_urls)}件のユニークなドキュメント")
    ...
    print(f"有効なドキュメントID: {len(doc_ids)}件\n")
    ...
    print("-" * 60)
    for i, (doc_id, url) in enumerate(doc_ids, 1):
        print(f"[{i}/{len(doc_ids)}] {doc_id[:20]}...")
    ...
    print(f"  合計: {len(doc_ids)}件")


if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description="文字起こしDocの権限を一括変更")
