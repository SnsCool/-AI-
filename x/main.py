...
SYSTEM_PROMPT = """あなたはAI関連ニュースを発信するアカウントです。元ツイートの内容と形式をほぼそのまま活かして、日本語で投稿文を作成してください。

【最重要ルール】
- 固有名詞（製品名、会社名、人名、サービス名など）は必ずそのまま使用すること
  例: ChatGPT, Claude, GPT-4, OpenAI, Anthropic, Google, Gemini など
- 元ツイートの内容・構成・トーンをできるだけ維持すること
- 元ツイートが英語の場合は日本語に翻訳すること

【禁止事項】
- URLやリンクは一切含めないこと
- 「詳細はこちら」等のリンク誘導文は含めないこと
- 架空の情報、存在しない発表などを捏造しないこと
- 元ツイートにない情報を追加しないこと
- 元ツイートにない見出し記号や装飾を追加しないこと"""
...
"""
    ...
    user_prompt = f"""以下のツイートの内容と形式をほぼそのまま活かして、日本語の投稿文を作成してください。
...
"""
    ...
    user_prompt += f"""
【重要な指示】
- 固有名詞（ChatGPT, Claude, OpenAI, Google等）は必ずそのまま使うこと
- 元ツイートの内容・構成をできるだけ維持すること
- 英語の場合は日本語に翻訳すること
- URLは含めないこと
...
【最重要】入力ツイートの内容を「文体スタイル」で書き換えること
- 入力ツイートと同じ意味・内容の投稿を、下記の「文体スタイル」で新しく書き換えること
- 固有名詞（製品名、会社名、人名など）は必ずそのまま使用すること
- 入力ツイートが英語の場合は日本語に翻訳すること
- 入力ツイートが日本語の場合も、文体スタイルに合わせて言い回しを変えること（丸コピーは禁止）
- 絵文字は削除すること
- 入力ツイートにない情報を追加しないこと

下記の「文体スタイル」を適用して、入力ツイートを書き換えてください。
...
### 入力ツイート（この内容を参考に新しい投稿を作成すること）:
{source_tweet}

【重要な指示】
- 上記の入力ツイートと「同じ意味・内容」の投稿を、上記の文体スタイルで「書き換えて」作成すること
- 固有名詞（製品名、会社名、人名など）は必ずそのまま使用すること
- 入力ツイートが英語の場合は日本語に翻訳すること
- 入力ツイートが日本語の場合は、文体スタイルに合わせて言い回しを変えること（丸コピーは禁止）
- 絵文字は削除すること
- 入力ツイートにない情報を追加しないこと
